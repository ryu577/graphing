...
#input: DAG g as an adjacent list representation 
# b is in a's adj list if a --> b
#construct transitive closure g' using DFS
#store a path dict (trans_path), where key = vertice v, value = a dict maps reachable nodes u to a path of v to u
#store a graph dict trans_dict (transitive closure of g, adjacent list representation)
...
#recover the path in the original graph from the path in the transitive graph, using the "trans_path" dict
def recover_path(g):
    original_paths = []
